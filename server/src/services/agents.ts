...
import { defaultAgentMaxConcurrentRuns, isUuidLike, normalizeAgentUrlKey } from "@paperclipai/shared";
...
export function agentService(db: Db) {
  ...
  return {
    ...
    create: async (companyId: string, data: Omit<typeof agents.$inferInsert, "companyId">) => {
      ...
      const runtimeConfig = normalizeRuntimeConfigForNewAgent(data.runtimeConfig, data);
    ...
    updatePermissions: async (id: string, permissions: { canCreateAgents: boolean; canRepairControlPlane?: boolean }) => {
      const existing = await getById(id);
      if (!existing) return null;
