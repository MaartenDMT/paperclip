// Run-log and run-event payload bounding helpers extracted from heartbeat.ts.
//
// These are pure utilities that cap the size of persisted run logs and run-event
// payloads so a runaway adapter cannot blow up storage or the live transcript.
// They hold no heartbeat state and are safe to use anywhere.

import { parseObject, appendWithByteCap, MAX_EXCERPT_BYTES } from "../../adapters/utils.js";
