...
async function writeFakePiCommand(commandPath: string): Promise<void> {
  const nodeScriptPath = process.platform === "win32" ? `${commandPath}.js` : commandPath;
...
`;
  ...
  await fs.chmod(nodeScriptPath, 0o755);
...
describe("pi_local execute", () => {
  it("fails the run when Pi exhausts automatic retries despite exiting 0", async () => {
    ...
    const commandPath = path.join(root, process.platform === "win32" ? "pi.cmd" : "pi");
