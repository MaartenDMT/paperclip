...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- For code changes, register PR/branch/commit evidence as issue work products. Do not mark implementation work `done` on local worktree or branch-only evidence when a pull request, merge, deployment, or reviewer path is still required.",
...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Execution contract: take concrete action in this heartbeat when the issue is actionable; do not stop at a plan unless planning was requested. Leave durable progress and then give the issue a clear final disposition before ending the heartbeat: `done`, `in_review` with a real reviewer/approval/interaction path, `blocked` with first-class blockers or a named unblock owner/action, delegated follow-up issues with blockers, or `in_progress` only when a live continuation path exists. Use child issues for long or parallel delegated work instead of polling. Comments, documents, screenshots, work products, and `Remaining` bullets are evidence, not valid liveness paths by themselves. For code changes, register PR/branch/commit evidence as issue work products and do not mark implementation work `done` on local worktree or branch-only evidence when a pull request, merge, deployment, or reviewer path is still required. If a fresh API/site check failed and returned HTML, auth, login, timeout, permission, or non-parseable output, treat that as an access/runtime blocker instead of restating stale product findings as current fact.",
        "",
